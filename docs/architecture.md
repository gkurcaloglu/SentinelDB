...
|---|---|
| `cmd/gateway` | Process entrypoint: loads config, wires everything together, accepts TCP connections, runs the metrics/API HTTP servers, handles graceful shutdown. |
| `internal/protocol` | Stateful PostgreSQL wire-protocol decoder (`Decoder`), message type tables, `RowDescription`/`DataRow` parsing and rebuilding, synthetic message builders (`ErrorResponse`, `ReadyForQuery`), the per-connection `TxState`, and `SerializedWriter` for safe concurrent writes to one client connection. |
| `internal/firewall` | `Gate`: the active client→server router. Applies a `Policy` to each frontend message, enforces V1 protocol scope (rejects TLS negotiation and the Extended Query Protocol), and writes synthetic `ErrorResponse`/`ReadyForQuery` for blocked queries. |
...
| `firewall.Gate` | Extended Query Protocol message (Parse/Bind/Describe/Execute/Close/Flush/Sync) | Rejected with `ErrorResponse` (`0A000`) and the connection is closed (`ErrUnsupportedProtocol`) rather than passed through unchecked. |
